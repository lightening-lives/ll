# PRODUCT — Lightening Lives LLP

## What it is
A Hyderabad-based (Telangana, India) genetic and molecular diagnostics LLP. It makes
population-scale genetic screening affordable and logistically simple enough to reach
tribal and rural communities that conventional diagnostics never reaches.

## Product truth
- **Sampling:** dried blood spot ("one-drop") collection — no venipuncture, no cold chain.
- **Core offer:** newborn screening and hereditary-disorder panels (haemoglobinopathies,
  inborn errors of metabolism), carrier screening, prenatal diagnosis.
- **Adjacent services:** preventive/community health programmes, R&D services,
  training & capacity building, partnerships & consultancy.
- **Founding thesis:** 25+ years in genetic/molecular diagnostics, with networks across
  clinicians, the national health ministry, district health authorities, NGOs and academia.
- **Framing:** the company explicitly organises itself around Sinek's Golden Circle —
  WHY, then HOW, then WHAT, in that order.

## Primary audience for THIS surface  (user-confirmed)
**Clinicians, labs and hospitals** — the people who order the assay and adopt the platform.
They are evaluating a supplier, not reading a mission statement. They need, in order:
what tests exist, what sample it takes, how fast the report comes back, how the logistics
work, and whether the analytical performance holds up. Mission earns the relationship;
specs win the evaluation.

Secondary, un-served-by-this-page: funders/CSR, government health departments, families.

## Primary action
**Request a test kit.** Secondary: download the test menu, book a platform walkthrough.

## Constraints
- Viewers include lab staff on mid-range Android hardware and uneven connectivity.
- Assay names, performance figures, accreditations and people are PLACEHOLDER until supplied.

## Assumptions (labelled, to be confirmed)
- A1 Sector, services, location, phone/email and the Golden Circle framing are taken from
  the company's own live site, read once to confirm the sector. Nothing else is reused.
- A2 All numbers, assay names, TATs, accreditations, staff and testimonials on the page
  are invented placeholders and are marked as such in `src/content.js`.
- A3 English-only for now; Telugu/Hindi not yet in scope.
